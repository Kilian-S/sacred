...
mkdir -p models/runs/gen28_fleet3
...
for S in 0 1 2; do
  OMP_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    scripts/train_aerial_generalist.py --sorties 24000 --eval-every 1000 --n-layouts-train 36 --seed $S --threads 2 \
    --json-out models/runs/gen28_fleet3/seed$S.json \
    --ckpt-dir models/runs/gen28_fleet3/seed${S}_ckpts \
    > models/runs/gen28_fleet3/seed$S.log 2>&1 &
  disown
done
echo "3 seeds detached; logs models/runs/gen28_fleet3/seed{0,1,2}.log"

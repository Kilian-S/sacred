...
mkdir -p models/runs/gen28_fleet
...
for S in 0 1 2; do
  OMP_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    --json-out models/runs/gen28_fleet/seed$S.json \
    --ckpt-dir models/runs/gen28_fleet/seed${S}_ckpts \
    > models/runs/gen28_fleet/seed$S.log 2>&1 &
  disown
done
echo "3 seeds detached; logs models/runs/gen28_fleet/seed{0,1,2}.log"

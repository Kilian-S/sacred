"""Export a gen45 DYNAMIC operations-map view: the trained policy + the adaptive enemy rolled
...
Oracle/eval-only. Output: models/runs/gen45_unified/dyn_view.json.
"""
import itertools
import json
import sys
...
from scratch.gen32_theatre_hunt import DynTheatre
from scratch.gen45_corridor_hunt import PATH, make_base, lethality_for, DOC32, TAU
...
FIELD, W = 45200, 2                      # a gated field; the gen45 window
Q_REP, Q_FLEE, Q_AR = DOC32
CKPT = "models/runs/gen45_unified/confirm_seed10_ckpts/actor_ep14000.pt"
T = 80
...
def main():
    base = make_base()
    th = base.th
    field = lethality_for(base, FIELD)
    ...
    json.dump(out, open("models/runs/gen45_unified/dyn_view.json", "w"))
    print("wrote gen45 dyn_view.json | field", FIELD, "routes", R, "sites", len(base.coords),
          "T", T)

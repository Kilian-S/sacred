...
against. Both respect the antagonist action mask (reach and budget), so they are directly
...
def mask_first_block_policy(event: DecisionEvent):
    """Deterministic "gateway" attacker: blocks the lexicographically-first maskable edge at max
    level. Under route reach the mask is exactly the edges on trucks' committed routes, so this
    is effectively an aimed attack despite its simplicity. Held out as a test attacker: training
    uses ``targeted_block_policy`` instead.
    """
    lbe = event.antagonist_action_mask.get("levels_by_edge", {})
    if not lbe:
        return None
...
def random_path_block_policy(smdp: SMDPDecisionWrapper, seed: int = 0):
    """Training attacker: blocks the first blockable edge on the path of a uniformly random
    goal-committed truck (falls back to a random maskable edge). Route-aimed like
    ``targeted_block_policy`` but stochastic across trucks, exposing the protagonist to broader
    interdiction patterns during training; ``targeted_block_policy`` (always the nearest-to-goal
    truck) stays held out as the test attack."""
    rng = random.Random(seed)
...
class ScriptedAttackerMixture:
    """A fixed weighted mixture of scripted attackers, one sampled per episode (deterministic by
    ``seed``). Training the defender against a mixture rather than a single fixed attacker exposes
    the policy to a diversity of interdiction patterns and avoids overfitting to one attacker.
    Stationary: no inner best-response training loop.

"""Contested-resupply arena: the config surface, which is the dynamic-assignment lineage plus
route reach, and the arena's wiring into the trainer CLI and the evaluation harness. No training,
though a short scripted-attacker rollout checks that the route-reach attacker engages in
destination mode."""
...
def test_contested_config_is_dynassign_plus_route_reach():
    c = contested_config()
    d = dynassign_config()
    # route reach is the only intended difference from the dynamic-assignment config
...
def test_route_reach_attacker_engages_in_destination_mode():
    """The route-reach targeted attacker spends budget blocking edges against greedy; a reach that
    fell back to empty in destination mode would spend nothing."""

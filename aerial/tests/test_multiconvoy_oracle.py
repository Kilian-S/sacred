...
def test_mission_gap_exceeds_linear_gap():
    """A loss-averse objective preserves the robustness gap where a risk-neutral one dilutes it."""
    from src.envs.multiconvoy_interdiction import make_multiconvoy_env
    ...
    assert linear.gap < 0.15        # deterministic spreading substitutes for mixing here
    assert mission.gap > 0.4

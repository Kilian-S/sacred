...
class AdaptiveLargeNeighborhoodSearchVRP:
    """Adaptive large neighbourhood search solver for the CVRP on graphs.

    Args:
        env: environment instance defining the topology and the truck speeds.
        decay: decay factor for the adaptive operator weights.
        annealing_temp: initial temperature for the simulated-annealing acceptance.
        cooling_rate: per-iteration cooling rate.
    """
    ...
    def __init__(
        ...
        # Each task is (customer_node, task_index_for_node, demand_value); a node's demand is split
        # into chunks of at most one truck capacity.
    ...
    def solve(self) -> dict[int, list[tuple[NodeId, int, float]]]:
        ...
        # ALNS convention is to destroy 20-30% of the tasks each iteration.
        ...
        for r in range(self.iterations):
            ...
            if candidate_cost < best_cost - 1e-6:
                ...
                # simulated-annealing acceptance of a worsening candidate
    ...
    def _nearest_neighbor_initial(self) -> dict[int, list[tuple[NodeId, int, float]]]:
        """Greedy nearest-neighbour task assignment heuristic."""
    ...
    def _destroy_worst(
        ...
        for _ in range(n_remove):
            ...
            # power-3 bias towards the worst-ranked tasks
    ...
    def _destroy_shaw(
        ...
        for _ in range(n_remove - 1):
            if not all_tasks:
                break
            # similarity = mean distance to the tasks already removed, so smallest is most similar
            similarities = []
            for task in all_tasks:
            ...
            # power-3 bias towards the most similar tasks
    ...
    def _repair_greedy(
        ...
        """Re-insert tasks into the positions that add the least travel distance."""
        for task in removed:

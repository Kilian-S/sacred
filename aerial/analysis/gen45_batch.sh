...
mkdir -p "$OUT"
# torch intra-op threads per run. 1 is the safe default when the machine is shared; set
# THREADS=2 when the wave has the machine to itself (3 runs x 2 = 6 of 10 cores). Thread
# count touches no game quantity.
T=${THREADS:-1}
case "$1" in
attempt)
  for S in 0 1 2; do

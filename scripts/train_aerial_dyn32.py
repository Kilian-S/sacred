#!/usr/bin/env python3
"""gen32: the gen31 dynamic-register trainer on the REAL Kaliningrad->Gvardeysk vec-theatre
(ledger experiments/gen32_theatre_dyn.md; Phase 0 pinned w=3, tau=0.10, doctrine
q=(0.6 repeat, 0.2 flee, 0.3 anti-repeat-anticipation)). Same SAC machinery as gen31; the only
changes are the substrate (real terrain via TheatreBase + TheatreEnv), the 3-component doctrine,
and w=3. Zero-shot layout = resampled hidden effectiveness field over the real site coordinates.

Refs (exact, per instance): iid_eq, local static optimum, the payoff-blind dynamic family
(anti-repeat/rotation over lanes+full+eq support), the fitted doctrine rules (myopic/soft/composed
dodge, disclosed caps), history_opt (lazy RVI). PRIMARY bar object = the static CAP
min(iid_eq, static_opt). Policy eval = EXACT stationary damage of the policy-induced window chain.
Dev-test = fields 5101/5102 (burned by Phase 0). GATED = 4100-4105 (pristine; --eval-gated only).
"""
...
from scratch.gen32_theatre_hunt import TheatreBase, DynTheatre, rule_family, resample_field

W, TAU, S_EP = 3, 0.10, 40
Q_REP, Q_FLEE, Q_AR = 0.6, 0.2, 0.3


class Inst:
    """Wraps a DynTheatre (oracle refs + doctrine + env) as a trainable instance."""

    def __init__(self, base: TheatreBase, name: str, seed: int):
        self.name = name
        self.g = DynTheatre(base, resample_field(base.coords, seed), W, TAU,
...
def make_pool(eval_gated=False):
    base = TheatreBase()
    train = [Inst(base, f"tr{1000 + s}", 1000 + s) for s in range(18)]
    val = [Inst(base, f"val{3000 + s}", 3000 + s) for s in range(4)]
    if eval_gated:
        test = [Inst(base, f"gated{4100 + s}", 4100 + s) for s in range(6)]
    else:
        test = [Inst(base, f"dev{s}", s) for s in (5101, 5102)]   # Phase-0-burned, diagnostics
...
def main():
    ...
    print("[gen32] building the real-theatre pool (exact refs per field)...", flush=True)
    t0 = time.time()
    ...
    if args.blind:                       # causal control: kill recency + doctrine columns
        for inst in train + val + test:
            inst._blind = True
    print(f"[gen32] pool in {time.time() - t0:.0f}s ({len(train)} train, {len(val)} val, "
    ...
    if args.sorties == 0:
        ...
        print(f"[gen32] UNTRAINED: test beats-CAP {beats}/{len(test)}; ratios-to-iid "
